      ...
      <SafeAreaView>
        <ScrollView>
          <View>
            ...
            <DatePicker />
            {showings && selected_film.id && <ShowingTimes film={selected_film} showings={showings} selected_date={selected_date} chooseTime={this.chooseTime} />}
            {films.map(film => (
              <FilmBrief film={film} key={film.id} isSelected={film.id === selected_film.id} />
            )
            )}
          </View>
          <Modal visible={showFilmDetails} onRequestClose={()=>{}}>
            <ScrollView>
